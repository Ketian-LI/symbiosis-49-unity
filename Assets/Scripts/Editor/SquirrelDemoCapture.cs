...
{
    public static class SquirrelDemoCapture
    {
        ...
        [MenuItem("Urban Wildlife/Capture Squirrel Animation Preview Frames")]
        public static void CapturePreviewFrames()
        {
            ...
            if (squirrel == null || camera == null)
            {
                throw new System.InvalidOperationException("Squirrel demo or layout camera was not generated.");
            }
            ...
            var basePosition = squirrel.SpawnPosition;
            var cameraFocus = basePosition + new Vector3(0.12f, 0.24f, 0.04f);
            camera.transform.position = cameraFocus + new Vector3(-2.3f, 1.75f, -2.6f);
            camera.transform.rotation = Quaternion.LookRotation(cameraFocus - camera.transform.position, Vector3.up);
            camera.orthographicSize = 1.33f;

            var projectRoot = Directory.GetParent(Application.dataPath)?.FullName ?? Application.dataPath;
            var previewDirectory = Path.Combine(projectRoot, "Previews", "SquirrelAnimation");
            ...
            for (var frame = 0; frame < FrameCount; frame++)
            {
                ...
                squirrel.ApplyPreviewPose(state, stateTime, position, rotation);
                RenderCamera(camera, Path.Combine(previewDirectory, $"frame_{frame:000}.png"));
            }

            File.Copy(
                Path.Combine(previewDirectory, "frame_018.png"),
                Path.Combine(projectRoot, "Previews", "SquirrelAnimationDemo.png"),
                true);
            Debug.Log($"[Squirrel Demo] {FrameCount} preview frames saved to {previewDirectory}");
        }

        private static void ResolvePreviewPose(
            float sequenceTime,
            Vector3 basePosition,
            out SquirrelDemoState state,
            out float stateTime,
            out Vector3 position,
            out Quaternion rotation)
        {
            position = basePosition + new Vector3(-0.48f, 0f, -0.10f);
            var travelVector = new Vector3(1.02f, 0f, 0.19f);
            rotation = Quaternion.LookRotation(travelVector.normalized, Vector3.up);
            if (sequenceTime < 0.75f)
            {
                state = SquirrelDemoState.Idle;
            ...
            {
                state = SquirrelDemoState.Forage;
