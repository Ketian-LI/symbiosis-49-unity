...
{
    public static class FoxFbxValidation
    {
        private const string AssetPath = "Assets/Resources/Animals/Fox/Fox_Animated_v01.fbx";

        [MenuItem("Urban Wildlife/Validate Blender Fox Asset")]
        public static void Validate()
        {
            ...
            {
                configuredClips[0].name = "Fox_Demo_12fps";
            ...
            if (prefab == null || meshes == 0 || clips.Length == 0)
            {
                throw new System.InvalidOperationException(
                    $"Fox FBX import is incomplete. Mesh renderers: {meshes}; animation clips: {clips.Length}.");
            }

            Debug.Log($"[Fox FBX] Import passed: {meshes} mesh renderers, {clips.Length} animation clip(s): {string.Join(", ", clips.Select(clip => clip.name))}.");
        }
    }
}
